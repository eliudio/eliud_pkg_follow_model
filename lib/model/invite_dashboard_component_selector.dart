...
import 'invite_dashboard_list_bloc.dart';
import 'invite_dashboard_list_event.dart';
...
class _SelectInviteDashboardWidgetState
  ...
  void _handlePrivilegeTabSelection() {
    if ((_privilegeTabController != null) &&
      ...
      BlocProvider.of<InviteDashboardListBloc>(context).add(
          InviteDashboardChangeQuery(
              newQuery: getComponentSelectorQuery(
                  _currentPrivilege, widget.app.documentID)));
    }
  }

  Widget theList(BuildContext context, List<InviteDashboardModel?> values) {
    var app = widget.app;
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<InviteDashboardListBloc, InviteDashboardListState>(
      ...
      if ((state is InviteDashboardListLoaded) && (state.values != null)) {
