...
import 'follow_request_list_bloc.dart';
import 'follow_request_list_event.dart';
...
class _SelectFollowRequestWidgetState extends State<_SelectFollowRequestWidget>
  ...
  void _handlePrivilegeTabSelection() {
    if ((_privilegeTabController != null) &&
      ...
      BlocProvider.of<FollowRequestListBloc>(context).add(
          FollowRequestChangeQuery(
              newQuery: getComponentSelectorQuery(
                  _currentPrivilege, widget.app.documentID)));
    }
  }

  Widget theList(BuildContext context, List<FollowRequestModel?> values) {
    var app = widget.app;
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<FollowRequestListBloc, FollowRequestListState>(
      ...
      if ((state is FollowRequestListLoaded) && (state.values != null)) {
